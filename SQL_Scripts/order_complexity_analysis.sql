...
SELECT oi_filter.items_per_order AS "Number of items in an order",
	COUNT(DISTINCT lo.order_id) AS "Number of unique late orders",
	COUNT(DISTINCT oi_filter.order_id) AS "Number of unique orders per number of items",
    ROUND((COUNT(DISTINCT lo.order_id) / COUNT(DISTINCT oi_filter.order_id)) * 100, 2) AS "Percent of late orders per number of items"
FROM (
	SELECT order_id,
		COUNT(order_item_id) AS items_per_order
	FROM order_items
...
LEFT JOIN late_orders lo
	ON oi_filter.order_id = lo.order_id
GROUP BY oi_filter.items_per_order
HAVING COUNT(DISTINCT oi_filter.order_id) > 100
ORDER BY oi_filter.items_per_order;
